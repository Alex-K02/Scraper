...
class ContentExtractor(object):
    ...
    def get_authors(self, doc):
        ...
        ATTRS = ['name', 'rel', 'itemprop', 'class', 'id', 'href']
        VALS = ['author', 'byline', 'dc.creator', 'byl', re.compile(r'^/authors/.*$')] #09.08.2024 new attribute and new value for futurism.com and mashable.com
        #TODO: check parsers for the case when vals is a regex function getElementsByTag
        matches = []
        authors = []
        ...
        for attr in ATTRS:
            for val in VALS:
                ...
                if isinstance(val, re.Pattern):
                    found = self.parser.getElementsByTag(doc, attr=attr, value=val, use_regex=True)
                    matches.extend(found)
                else:
                    found = self.parser.getElementsByTag(doc, attr=attr, value=val, use_regex=False)
                    matches.extend(found)
